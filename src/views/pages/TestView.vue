        ...
        <div
            ...
            <div class="h-[600px] overflow-auto flex flex-col gap-2 ">
                <div v-for="history in pushedHistoryChat" @click="textArea = history"
                    class="bg-slate-100 rounded-lg cursor-pointer p-2"> {{ history }}</div>
            </div>
        ...
        <div
            ...
            <div class="flex justify-between h-20 ">
                <div class="text-2xl flex items-center pb-8">
                    cheat<span :class="indikator ? 'animate-bounce' : 'animate-none'"><b> GPU 4.0</b></span> <n-icon
                        color="#">
                        <verify-icon />
                    </n-icon>
                </div>
...
const proPel = async (e) => {
    ...
    if (!response.ok) {
        ...
        const date = Date.now();
        pushedInvoice.value.unshift({ no_invoice: bentukReport, tgl: date, stts: true, type: "pelunasan" });
        count.value += 1;
    }
